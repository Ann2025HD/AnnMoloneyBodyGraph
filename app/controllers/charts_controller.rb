...
PLANET_PNG_BY_KEY = {
  ...
  def download_prawn
    ...
    debug_flag = Rails.env.production? ? "false" : "true"
    cmd = [
      node_bin, cli.to_s,
...
# =========================
   ...
   #----------------
    ...
    pdf.fill_color "666666"; pdf.text "Generated by Ann Moloney Human Design 2026   www.annmoloney.com", size: 8, align: :center
    pdf.fill_color "000000"
  ...
  def normalize_time(val)
    ...
    # First: handle "HH:MM:SS" or "H:MM:SS" by ignoring seconds
    if s =~ /\A(\d{1,2}):(\d{2}):\d{2}\z/
        h = $1.to_i
        m = $2.to_i
        return nil if h > 23 || m > 59
        return format("%02d:%02d", h, m)  # => "14:27"
    end
